...
class BaseAnimal(Agent):
    ...
    def _move(self) -> None:
        ...
        new_position = (
            self.pos[0] + np.cos(angle) * distance,
    ...
    def _local_sense(self, pos, vision: int):
        neighborhood = self.model.space.get_neighbors(
            pos, radius=vision, include_center=True
        )
        counts = {"plants": 0, "prey": 0, "predators": 0}
        nearest = {"plants": None, "prey": None, "predators": None}
        for obj in neighborhood:
            dist = self._torus_distance(pos, obj.pos)
    ...
    def _move_with_scoring(
        ...
        for _ in range(12):
            angle = self.model.random.random() * 2 * np.pi
            distance = self.model.random.random() * radius
            pos = (
                self.pos[0] + np.cos(angle) * distance,
                self.pos[1] + np.sin(angle) * distance,
            )
            candidates.append(self.model.space.torus_adj(pos))
        if allow_stay:
            candidates.append(self.pos)
        best_score = None
        best_positions = []
        for pos in candidates:
            terrain = self.model.terrain_at(pos)
            counts, nearest = self._local_sense(pos, vision=vision)
            score = score_fn(counts, nearest, terrain)
            score += (self.model.random.random() - 0.5) * 0.5
...
class Prey(BaseAnimal):
    ...
    def _eat(self) -> None:
        ...
        plants = [obj for obj in neighbors if isinstance(obj, Plant) and obj.available]
        if plants:
            plant = self.model.random.choice(plants)
            if plant.consume():
...
class Predator(BaseAnimal):
    ...
    def _eat(self) -> None:
        neighbors = self.model.space.get_neighbors(self.pos, radius=EAT_RADIUS, include_center=True)
        prey_agents = [a for a in neighbors if isinstance(a, Prey)]
        competitor_agents = [a for a in neighbors if isinstance(a, Competitor)]
        targets = prey_agents + competitor_agents
        if targets:
            target = self.model.random.choice(targets)
            self.model.space.remove_agent(target)
...
class EcosystemModel(Model):
    ...
    def _init_plants(self, count: int, regrow_delay: int) -> None:
        for _ in range(count):
            ...
            adjusted_delay = max(1, int(round(regrow_delay * TERRAIN_REGROW_MULT[terrain])))
            plant = Plant(self._next_id(), self, adjusted_delay)
            self.space.place_agent(plant, (x, y))
    ...
    def _init_competitors(self) -> None:
        for _ in range(self.initial_competitors):
            ...
            x = self.random.random() * self.space.width
            y = self.random.random() * self.space.height
            self.space.place_agent(competitor, (x, y))
    ...
    def _init_terrain(self) -> Dict[tuple, str]:
        weights = [self.terrain_weights[t] for t in TERRAIN_TYPES]
        total = sum(weights)
        weights = [w / total for w in weights]
        terrain_map: Dict[tuple, str] = {}
        for x in range(int(self.space.width)):
            for y in range(int(self.space.height)):
                terrain_map[(x, y)] = self.random.choices(TERRAIN_TYPES, weights=weights, k=1)[0]
        return terrain_map

    def terrain_at(self, pos) -> str:
        x = int(pos[0]) % int(self.space.width)
        y = int(pos[1]) % int(self.space.height)
        return self.terrain.get((x, y), "plain")

    def terrain_move_cost(self, pos) -> int:
        return TERRAIN_MOVE_COST[self.terrain_at(pos)]

    def terrain_grid(self) -> np.ndarray:
        grid = np.zeros((int(self.space.height), int(self.space.width)), dtype=int)
        terrain_index = {name: idx for idx, name in enumerate(TERRAIN_TYPES)}
        for (x, y), terrain in self.terrain.items():
            grid[y, x] = terrain_index[terrain]
        return grid
...
def run_visual_simulation(
    ...
    ax_grid.set_xlim(0, model.space.width)
    ax_grid.set_ylim(0, model.space.height)
    ax_grid.set_aspect("equal")
    ax_grid.set_title("Ecosystem Grid")
    ax_grid.set_xticks([])
    ax_grid.set_yticks([])

    terrain_colors = ["#d9c6a5", "#6d9f71", "#b08d57", "#8b6f5a"]
    terrain_grid = model.terrain_grid()
    ax_grid.imshow(
        terrain_grid,
        cmap=plt.matplotlib.colors.ListedColormap(terrain_colors),
        origin="lower",
        alpha=0.35,
    )
    ...
    legend_handles = [
        ...
        Patch(facecolor="#d9c6a5", label="Plain"),
        Patch(facecolor="#6d9f71", label="Forest"),
        Patch(facecolor="#b08d57", label="Hill"),
        Patch(facecolor="#8b6f5a", label="Mountain"),
    ]
